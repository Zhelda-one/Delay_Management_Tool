...
function UlDatadecodepucchAreaBoundaries_t(offset) {
    ...
    result.pucchBoundary = decodeStaticFixedSizedArray_pucchBoundary_t_4(offset + 4);
    result.eCpriConfig = UlDatadecodeeCpriConfigPucch_t(offset + 28);

    return result;
}
function UlDataencodepucchAreaBoundaries_t(msg, buf, off) {
    l2l1_putU8(msg.numPucchBoundaries, buf, off + 0);
    encodeStaticFixedSizedArray_pucchBoundary_t_4(msg.pucchBoundary, buf, off + 4);
    UlDataencodeeCpriConfigPucch_t(msg.eCpriConfig, buf, off + 28);
}
...
function UlDatadecodepucchReceiveReqSubcell_t(offset) {
    ...
    result.pucchResources = decodeDynamicVariableSizedArray_pucchReceiveReqPucchResource_t_252(offset + 88);
...
function UlDataencodepucchReceiveReqSubcell_t(msg, buf, off) {
    ...
    encodeDynamicVariableSizedArray_pucchReceiveReqPucchResource_t_252(msg.pucchResources, buf, off + 88);
}
function UlDatadecodepucchReceiveReqPucchResource_t(offset) {
    let result = {};
...
function encodeStaticFixedSizedArray_pucchBoundary_t_4(arr, buf, off) {
    for (let i = 0; i < 4; i++)
        UlDataencodepucchBoundary_t(arr[i], buf, off + i * 6);
}
function decodeStaticFixedSizedArray_pucchBoundary_t_4(offset) {
    const result = [];
    for (let i = 0; i < 4; i++)
